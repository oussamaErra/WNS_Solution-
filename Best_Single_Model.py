...
for col in categorical_features:
    items_data = pd.merge(items_data,encode_categories_items(col,items_data),on=col,how ='left')
...
def app_code_features(train,test,view_logs,items_data):
    ...
    all_data_gb = all_data.groupby('app_code').agg({"app_code":['size'],
                                      #"is_4G":['nunique'],
                                      #"os_version":["nunique"],
                                      "session_id":["nunique"],
                                      "item_id":["nunique"],
                                      ...
                                      'device_type':{'m_c': lambda x: Counter(x).most_common(1)[0][0],'nq': lambda x: len(np.unique(x))},
                                      'category_1': {'m_c': lambda x: Counter(x).most_common(1)[0][0], 'nq': lambda x: len(np.unique(x))},
                                      'category_2': {'m_c': lambda x: Counter(x).most_common(1)[0][0], 'nq': lambda x: len(np.unique(x))},
                                      'category_3': {'m_c': lambda x: Counter(x).most_common(1)[0][0], 'nq': lambda x: len(np.unique(x))},
                                      'product_type': {'m_c': lambda x: Counter(x).most_common(1)[0][0], 'nq': lambda x: len(np.unique(x))},
...
def session_id_features(view_logs,items_data):
    ...
    all_data_gb = all_data.groupby('session_id').agg({'user_id':['nunique'],'server_time':{'life':lambda x: (np.max(x) -np.min(x))/np.timedelta64(1,'h')},
                                                      "item_id": ["nunique"]})#,'server_hour':['nunique'],'server_day':['nunique'] })
...
def user_features(view_logs,items_data,train,test):
    ...
    all_data_gb = all_data.groupby('user_id').agg({
                                                       ...
                                                       'category_2': {'m_c': lambda x: Counter(x).most_common(1)[0][0], 'nq': lambda x: len(np.unique(x))},
                                                       'category_3': {'m_c': lambda x: Counter(x).most_common(1)[0][0], 'nq': lambda x: len(np.unique(x))},
                                                       'product_type_size': {'m_c': lambda x: Counter(x).most_common(1)[0][0], 'nq': lambda x: len(np.unique(x))},
                                                        'product_type_itm_cnt': {'m_c': lambda x: Counter(x).most_common(1)[0][0], 'nq': lambda x: len(np.unique(x))},
                                                         'product_type_pr_mn': {'m_c': lambda x: Counter(x).most_common(1)[0][0], 'nq': lambda x: len(np.unique(x))},
                                                         'product_type': {'m_c': lambda x: Counter(x).most_common(1)[0][0], 'nq': lambda x: len(np.unique(x))},
                                                        'category_1_size': ['mean'],
                                                        ...
                                                        'item_id_nunique_session': ['max','mean','nunique'],
                                                         'server_hour':{'m_c': lambda x: Counter(x).most_common(1)[0][0],\
                                                        ...
                                                        #'server_day': {
                                                            ...
                                                            #'mean': lambda x: np.mean(x)
                                                       # },
                                                        'server_time':{'life':lambda x: (np.max(x) -np.min(x))/np.timedelta64(1,'h')}
...
def encode_feat_by_feat(d_train, public_set, private_set, test, to_encode, to_use_to_encode, function='mean'):
    dict_encode = d_train.groupby(to_encode).agg({to_use_to_encode: function}).to_dict()[to_use_to_encode]
    d_train[to_encode + '_' + to_use_to_encode] = d_train[to_encode].apply(lambda x: dict_encode.get(x, 0))
    test[to_encode + '_' + to_use_to_encode] = test[to_encode].apply(lambda x: dict_encode.get(x, 0))
    if public_set is not None:
        public_set[to_encode + '_' + to_use_to_encode] = public_set[to_encode].apply(lambda x: dict_encode.get(x, 0))
        private_set[to_encode + '_' + to_use_to_encode] = private_set[to_encode].apply(lambda x: dict_encode.get(x, 0))
...
def process_impressions_data(impress_data):
    impress_data['impression_time_hour'] = pd.to_datetime(impress_data.impression_time).dt.hour
...
def return_publc_private(train,view_logs):
    ...
    public_set =train.loc[(train.impression_time>start_public) & (train.impression_time<=end_public) ]
    private_set=train.loc[train.impression_time>end_public]
    return d_train,public_set,private_set,view_logs[view_logs.server_time < limit_view_log]
...
def encode_target_features(d_train, public_set, private_set, test, col, target):
    ...
    for t, v in skf.split(d_train, d_train[col]):
        ...
        v_train[col + '_encode_target'] = v_train[col].apply(lambda x: encod_dict.get(x, float('nan')))
        train_list.append(v_train)
    encod_dict_train = d_train.groupby(col).agg({target: 'mean'}).to_dict()[target]
    test[col + '_encode_target'] = test[col].apply(lambda x: encod_dict_train.get(x, float('nan')))
    if public_set is not None:
        public_set[col + '_encode_target'] = public_set[col].apply(lambda x: encod_dict_train.get(x, float('nan')))
        private_set[col + '_encode_target'] = private_set[col].apply(lambda x: encod_dict_train.get(x, float('nan')))
...
def train_test_features(train, test):
    ...
    group_pass = data_all.groupby(['hour_impression','day_impression','user_id']).size().\
                            to_frame('size_userd_per_day').reset_index()
    data_all = pd.merge(data_all,group_pass,on=['user_id','day_impression','hour_impression'],how='left')


    group_pass = data_all.groupby(['hour_impression','day_impression','app_code', 'user_id']).size().\
                            to_frame('size_user_per_app_code_per_hour').reset_index()
    data_all = pd.merge(data_all, group_pass, on=['app_code', 'user_id','hour_impression','day_impression'], how='left')



    group_pass = data_all.groupby(['hour_impression', 'day_impression', 'user_id']).\
                            agg({'app_code':{'n_q_app_per_user_per_hour':'nunique','app_code_sz_by_nq_per_h':lambda x: x.shape[0]/len(np.unique(x))}}).reset_index()
    data_all = pd.merge(data_all, group_pass, on=['user_id', 'hour_impression', 'day_impression'], how='left')

    group_pass = data_all.groupby(['day_impression', 'user_id']).agg({'app_code':\
                            { 'n_q_app_per_user_per_day':lambda x : len(np.unique(x)),'app_code_sz_by_nq_per_day':lambda x: x.shape[0]/len(np.unique(x))}})\
                            .reset_index()
    data_all = pd.merge(data_all, group_pass, on=['user_id', 'day_impression'], how='left')
    ...
    data_all['n_show_plalt'] = data_all.groupby('app_code')['app_code'].transform('size')
    data_all['ratio_type_1'] = data_all['n_show_plalt']/data_all['n_show_user']
    return data_all.iloc[:n_train, :], data_all.drop('is_click', axis=1).iloc[n_train:, :].reset_index(drop=True)
...
#########################################################################
cols_to_labelencode.extend(['os_version', 'device_type_m_c_user',
       'device_type_m_c_app'])
encoders ={}
for col in set(cols_to_labelencode).intersection(set(d_train.columns)):
    encoders[col] = LabelEncoder()
    encoders[col].fit(train[col].map(str).values.tolist()+test[col].map(str).values.tolist())
...
for col in set(cols_to_mean_price_encode).intersection(set(d_train.columns)):
    d_train, public_set, private_set, test = encode_feat_by_feat(d_train, public_set, private_set, test, col, 'item_price_mean_user', function='mean')
...
if use_target_encoding:
    d_train,public_set,private_set,test= encode_target_features(d_train,public_set,private_set,test,'app_code','is_click')
...
for i,(t_index,val_index) in enumerate(skf.split(d_train,d_train[target[0]])):
    df_train = lgbm.Dataset(d_train.drop(cols_to_drop+target,axis=1).iloc[t_index,:] ,d_train[target[0]].values[t_index])
    df_val   = lgbm.Dataset(d_train.drop(cols_to_drop+target,axis=1).iloc[val_index,:],d_train[target[0]].values[val_index])

    model = lgbm.train(params, df_train, num_boost_round=10000, valid_sets=[df_train, df_val], valid_names=['train', 'val'], early_stopping_rounds=150)
    oof[val_index] = model.predict(d_train.drop(cols_to_drop + target, axis=1).iloc[val_index, :], num_iteration=model.best_iteration)
    validation_score.append(model.best_score['val']['auc'])
    if public_set is not None:
        preds_private.append(model.predict(private_set.drop(cols_to_drop + target, axis=1)))
        preds_public.append(model.predict(public_set.drop(cols_to_drop + target, axis=1)))
        scores_private.append(roc_auc_score(private_set[target[0]].values,preds_private[i]))
        scores_public.append(roc_auc_score(public_set[target[0]].values,preds_public[i]))
        print('the auc on the private set is {}'.format(scores_private[i]))
        print('the auc on the public set is {}'.format(scores_public[i]))
    else:
        preds_test.append(model.predict(test.drop(cols_to_drop , axis=1),num_iteration=model.best_iteration))
...
if public_set is not None:
    ...
    print('the cross validation score of public is {}'.format(roc_auc_score(public_set[target[0]].values, np.mean(preds_public, axis=0))))
    print('--------------------Private----------------------------------------------------------')
    print('the private scores are {}'.format(scores_private))
    print('the cross validation score of private is {}'.format(roc_auc_score(private_set[target[0]].values,np.mean(preds_private,axis=0))))
    ...
    try:
        ...
        pass
    test[['impression_id', 'is_click']].to_csv(f'{file_name}/{file_name}_test.csv', index=None)
    a =pd.DataFrame()
